...
namespace MetaNN
{
	namespace NSBiasVector
	{
		...
		public:
			...
			EvalItem(TInputHandle inputHandle,TOutputHandle outptHandle,size_t vecLen,size_t pos)
				:BaseEvalItem(TypeID<EvalItem>(), {inputHandle.DataPtr()},outputHandle.DataPtr())
		...
		{
			using EvalItemType = EvalItem<TInputHandl, TOutptHandle>;
		protected:
	...
	public:
		...
		auto EvalRegister() const
		{
			if (!m_evalBuf.IsEvaluated()) {
				...
				if (!EvalPlan::Inst().IsAlreadyRegisted(outHandle.DatraPtr())) {
