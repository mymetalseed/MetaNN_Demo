...
namespace MetaNN
{
	...
	public:
		...
		explicit Tensor(TShapeParameter... shape)
			: m_shape(shapes...)
			, m_mem(m_shape.Count())
		{}

		explicit Tensor(ContinuousMemory<ElementType, DeviceType> p_meme, MetaNN::Shape<uDim> p_shape)
		...
		{
			static_assert(std::is_same_v<DeviceType, DeviceTags::CPU>, "Only CPU supports this method.");
	...
	{
		LowerAccessImpl(Tensor<TElemnt,TDevice,uDim> p)
			:m_data(std::move(p))
		{}
